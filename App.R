...
server <- function(input, output, session) {
  ...
  observe({
    if (is.null(input$focal_y) ||
      input$focal_y == "" ||
      is.null(input$focal_x) ||
      input$focal_x == "") {
  ...
  observeEvent(input$go, {
    showModal(
      modalDialog(
        HTML("Preparing data for your focal site - this will take several minutes. ☕🥱🍪"),
        footer = NULL
      ))
    ...
    focal_watershed_ID = focal_watershed %>%
      dplyr::select(PRV_WSHD_I, NHN_TWS_ID) %>%
    ...
    npri_release_clip = npri_release_clip %>%
      group_by(NPRI_Site_ID) %>%
      summarise(
    ...
    npri_labels <- lapply(seq(nrow(npri_clip)), function(i) {
      paste0(
        "data/National Pollutant Release Inventory", "<br>",
    ...
    if (input$watershed_scale == "NHN Tertiary Watersheds") {
      fp <- paste0(
        "data/Human modification/GHM_17_NHN/GHM17_", focal_watershed_ID,
        "/GHM17_", focal_watershed_ID, ".tif"
      )
    } else {
      fp <- paste0(
        "data/Human modification/GHM_17_PRV/GHM17_", focal_watershed_ID,
    ...
    if (input$watershed_scale == "NHN Tertiary Watersheds") {
      sent <- paste0(
        "data/Sentinel_LULC_v2/Sent21_NHN/", focal_watershed_ID,
        "/Sen_", focal_watershed_ID, "_3857.tif"
      )
    } else {
      sent <- paste0(
        "data/Sentinel_LULC_v2/Sent21_PRV/", focal_watershed_ID,
        "/Sen_", focal_watershed_ID, "_3857.tif"
      )
    }
    ...
    data_t1 <- reactive({
      ...
      tmp = tmp %>%
        dplyr::select(Common_name, Scientific_name, Species_origin) %>%
        arrange(Common_name)
    ...
    n_barrier = nrow(fishways_clip) +
      nrow(wfs_clip) +
      nrow(dams_clip)
    n_ind = nrow(mines_clip) +
      nrow(metalworks_clip) +
      nrow(oilgas_clip) +
      nrow(npri_clip) +
      nrow(wastewater_clip) +
      nrow(contaminated_sites_clip)
