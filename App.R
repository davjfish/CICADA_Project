...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    p(
      ...
      displays spatial information on fish and fish habitat in freshwaters across Canada."
    ), br(),
    p("For guidance on using this tool, please consult the user manual available here."),
    br(),
    p(
      "Development and maintenance of this tool is led by the Chu Lab in DFO's Great Lakes Laboratory for Fisheries and Aquatic Sciences. Funding for development of CICADA was provided by DFO's Competitive Science Research Fund. For inquiries related to this project or the web app 
      please contact Cindy Chu (cindy.chu@dfo-mpo.gc.ca)."
    ),
    br(),
    p(
      "This tool relies on open data shared by many government and 
...
server <- function(input, output, session) {
  ...
  observeEvent(input$go, {
    showModal(
      modalDialog(
        HTML("Preparing data for your focal site - this could take up to 5 minutes."),
        footer = NULL
      ))
